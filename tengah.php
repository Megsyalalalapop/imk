...
if ($_GET[module]=='store'){
     ...
     echo " 
            </div><div class='profil2'><b>Selamat Datang di Restaurant Beta Mangan</b><br />
<p>Kami menyediakan berbagai makanan khas Medan sangat cocok dinikmati saat anda lapar dan haus.<br>
Selamat memesanan. Dan terima kasih atas kunjungan anda.</p> </div>
          </div> "; 
  ...
  while ($r=mysql_fetch_array($sql)){
    ...
    $stok=$r['stok'];
    $tombolpesan="<a class='prod_cart' href=\"aksi.php?module=keranjang&act=tambah&id=$r[id_produk]\">PESAN</a>";
    ...
    $hargatetap="<div class='prod_price'><span class='price'> <br /></span>&nbsp;<span class='price'> Rp. <b>$harga,-</b></span></div>                        
          </div>";
...
elseif ($_GET[module]=='detailproduk'){
	...
	echo"<div class='feat_prod_box_details'>";
	            ...
	            echo"
				<div class='product_img'>
                             <a href='produk-$d[id_produk]-$d[produk_seo].html'><a href='foto_produk/$d[gambar]' rel='clearbox[gallery=Koleksi Produk]' title='$d[nama_produk]'>
...
if ($_GET['module']=='profilkami'){
  ...
  echo "<h4 class='heading colr'>Profil Kami</h4>
          ...
          <div class='profil'>
              <div><p>Beta Mangan - Makanan Online Khas Medan<br>
              Beta Mangan merupakan website jajanan online makanan medan, Beta Mangan lebih mengacu ke jajanan tradisional yang jarang ditemukan atau bisa di katakan sulit untuk didapatkan. Untuk memudahkan pencinta kuliner tradisional medan. Beta Medan menyediakan jajanan tradisional medan </p>

<p>Akhirnya, kami mengucapkan terima kasih atas kunjungan anda di Beta Mangan.</p></div>
...
// Modul cara pempesanan
if ($_GET['module']=='carapesan'){
  // Data cara pempesanan mengacu pada id_modul=45
	$cara = mysql_query("SELECT * FROM modul WHERE id_modul='45'");
	$r      = mysql_fetch_array($cara);

  echo "<h4 class='heading colr'>Cara Pempesanan</h4>
...
elseif ($_GET[module]=='keranjangbelanja'){
  ...
  if($ketemu < 1){
    echo "<script>window.alert('Pesanannya masih kosong. Silahkan Anda memesan terlebih dahulu');
        window.location=('index.php')</script>";
    }
  else{  
  
    echo "<h4 class='heading colr'>Pesanan</h4>
  ...
  echo "<tr><td colspan=5 align=right><br><span class='table3'>Total:</td><td colspan=2><br><span class='table3'>Rp. $total_rp,-</b></td></tr>
        <tr><td colspan=2><br /> <a href=semua-produk.html style='width: 100px; height: 16px;' type=submit class=simplebtn>LANJUT MEMESAN</a><br /></td>


        <td colspan=2><br /><input style='width: 130px; height: 21px;' type=submit  class= simplebtn value='UPDATE PESANAN'><br /></td>
        </tr>
        </tbody>
  </table>";
echo "<br /><br /><br /><br /><p>*   Apabila Anda mengubah jumlah (Qty), jangan lupa tekan tombol <b>Update Pesanan</b><br />
               **  Total harga di atas belum termasuk ongkos kirim yang akan dihitung saat pesanan diantar ke alamat.</p><br />
              </div>
          </div>    
          </div>
...
if ($_GET['module']=='selesaibelanja'){
  ...
  $ketemu=mysql_num_rows($sql);
  if($ketemu < 1){
   echo "<script> alert('Pesanan masih kosong');window.location='index.php'</script>\n";
   	 exit(0);
	}
	else{


  echo "<h4 class='heading colr'>Data Pemesan</h4>
      ...
      <tr><td colspan=2><input style='width: 60px; height: 25px;' type=submit class= simplebtn value=PROSES></td></tr>
      </table>";
		  
     echo "<h4 class='heading colr'>Konfirmasi Pesanan Anda</h4>
          <table width=670 border=0 cellpadding=0 cellspacing=1 align=center>
          <tbody>
...
//END nampilkan diskon per produk --
  ...
  echo "<tr><td></td><td></td>
    ...
    echo "<div class='prod_box_big'>
          ...
          <span style='float : right;'><input style='width: 110px; height: 22px;' type=submit  class= simplebtn value='PROSES PESANAN'></span></td></tr></table>
          </div></div></div>
...
if (empty($_POST[nama]) || empty($_POST[alamat]) || empty($_POST[telpon])){
...
mysql_query("INSERT INTO orders(nama_kustomer, alamat, telpon, tgl_order, jam_order) 
             VALUES('$_POST[nama]','$_POST[alamat]','$_POST[telpon]','$tgl_skrg','$jam_skrg')");
...
for ($i = 0; $i < $jml; $i++) {
  ...
  echo "<h4 class='heading colr'>Proses Transaksi Selesai</h4>";
      ...
      <tr><td>Telpon         </td><td> : $_POST[telpon] </td></tr></table><br />
